...
export const Footer = () => {
  return (
    <div
      ...
      className="h-screen flex w-full py-20 px-10 bg-zinc-900 overflow-hidden"
    >
      <div className="w-1/2 gap-[30rem] flex flex-col h-screen z-0">
        <div className=" justify-evenly">
          <h1 className="text-[7rem] leading-none tracking-tighter text-white font-bold">
            EYE-
          </h1>
          <h1 className="text-[7rem] leading-none tracking-tighter text-white font-bold">
            OPENING
      ...
      <div className="w-1/2 pr-10 h-screen">
        <h1 className="text-[7rem] leading-none tracking-tighter text-white font-bold">
          PRESENTATIONS
        </h1>
        <div className="flex mt-10 justify-between">
          <div className="flex gap-10 flex-col">
            <div>
              <p className="text-xl font-extralight mb-5">S:</p>
              {["Instagram", "Facebook", "Twitter", "LinkedIn"].map(
                (val, index) => (
                  <p className="text-xl underline mt-2" key={index}>{val}</p>
                )
              )}
            </div>
            <div>
              <p className="text-xl font-extralight mb-5">L:</p>
              {[
                "202-1965 W 4th Ave",
                "Vancouver, Canada",
                "30 Chukarina St",
            ...
            <div>
              <p className="text-xl font-extralight mb-5">E:</p>
              {["hello@ochi.design"].map((val, index) => (
                <p  className="text-xl underline mt-2" key={index}>{val}</p>
              ))}
            </div>
          </div>
